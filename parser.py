...
def parse_duration(value: str):
    ...
    Only accepts values that explicitly contain:
...
def detect_header_row(df: pd.DataFrame) -> int:
    """
    Detect header row based on percentage-like columns.
    Looks for row containing at least 3 % values.
    """
...
def detect_duration_column(df_table: pd.DataFrame) -> int:
    """
    Detect duration column based on actual cell content.
    Finds column with highest count of valid duration values.
    """
...
def clean_and_validate_rows(df, duration_col, premium_mapping):
    ...
    # Clean duration column (POSITIONAL)
    ...
    for _, row in df.iterrows():
        ...
        if (
            ...
            if invalid_streak >= MAX_INVALID_STREAK:
                logger.info("Footer detected after consecutive invalid rows. Stopping parse.")
                break
